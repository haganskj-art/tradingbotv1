...
V6 uses a selective confluence model rather than a single z-score trigger. It evaluates 1-minute data with 5-minute and 15-minute trend confirmation, EMA alignment, session VWAP, RSI/momentum, pullback/reclaim behavior, candle quality, relative volume, and an overextension filter. A candidate must also confirm on the prior completed bar and respect a short cooldown.

The displayed quality score is a rule-based confluence score, not a probability of profit or a guarantee of accuracy. The default threshold is intentionally strict to reduce low-quality alerts.

The built-in Yahoo Finance CME futures feed is delayed. For active trading, use an authorized real-time market-data source and validate signals in simulation before relying on them.
